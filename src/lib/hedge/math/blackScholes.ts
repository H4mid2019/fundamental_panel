...
/**
 * Find the strike whose |delta| is closest to a target, interpolating between
 * the two bracketing listed strikes.
 *
 * Real chains are quantized — a name with $5 strike spacing may jump from 0.22
 * to 0.31 delta with nothing between — so returning the nearest *listed* strike
 * would make the "25-delta IV" a different moneyness for every ticker, and the
 * cross-sectional skew comparison meaningless. Interpolating the IV between the
 * two bracketing strikes gives a consistent 25-delta point on every chain. The
 * nearest listed strike is returned alongside, for scanners that must trade a
 * real contract.
 *
 * @param points - Listed strikes with their IV and |delta|, in any order.
 * @param targetDelta - Absolute delta to solve for (e.g. 0.25).
 * @returns The interpolated IV and the nearest tradable strike, or `null` when
 *   the chain does not bracket the target.
 */
export function interpolateAtDelta(
  points: readonly { strike: number; iv: number; absDelta: number }[],
  targetDelta: number,
): { iv: number; nearestStrike: number; bracketed: boolean } | null {
  const usable = points
    .filter(
      (p) =>
  ...
  let below: (typeof usable)[number] | undefined;
  let above: (typeof usable)[number] | undefined;
  for (const p of usable) {
    if (p.absDelta <= targetDelta) below = p;
    if (p.absDelta >= targetDelta && !above) above = p;
  }

  // The chain does not straddle the target: fall back to the closest point we
  // do have, but say so, so callers can discount a non-bracketed reading.
  if (!below || !above) {
  ...
  const span = above.absDelta - below.absDelta;
  const w = span <= 0 ? 0 : (targetDelta - below.absDelta) / span;
  const iv = below.iv + w * (above.iv - below.iv);
  const nearestStrike = w < 0.5 ? below.strike : above.strike;
  return { iv, nearestStrike, bracketed: true };
}
