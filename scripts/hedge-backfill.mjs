#!/usr/bin/env node
/**
 * Rebuild HedgeScope history against a running server.
 *
 *   npm run hedge:backfill                      # localhost:3000
 *   HEDGE_URL=http://127.0.0.1:3500 npm run hedge:backfill
 *
...
try {
  const response = await fetch(`${base}/api/hedge/backfill`, {
    method: "POST",
  ...
  out(
    "    Realized-vol history for all tickers; real IV for SPY/QQQ/DIA/GLD/USO.",
  );
} catch (error) {
